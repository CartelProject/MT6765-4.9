...
int eta6937_chargin_hw_init_done = 0;
...
#else
static int eta6937_get_chip_status(struct charger_device *chg_dev, bool *done)
{
	unsigned int ret = 0;
	unsigned char val = 0;
...
int eta6937_enable_otg_ex( bool en)
{
#if defined(CONFIG_USB_MTK_OTG)
    if(yk_otg_enable==0 && en == 1)
    {
...
static int eta6937_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_eta6937_info = info;
	client->addr = (eta6937_SLAVE_ADDR_WRITE>>1);
	new_client = client;
	info->dev = &client->dev;
	info->chg_dev_name = "primary_chg";
	...
	/* eta6937_hw_init(); //move to charging_hw_xxx.c */
		...
		#if defined(CONFIG_TERACUBE_2E)
         eta6937_reg_config_interface(0x06,0x7F); //7A
		 eta6937_reg_config_interface(0x02,0xFE); //B6
		#else
